...
from miranda.convert._data_corrections import load_json_data_mappings, dataset_conversion
...
def get_ghcn_raw(station_ids: list=None, stationtype:str=None, outfolder: Path=None, timeout:int=None) -> None:
...
def convert_ghcn_bychunks(
    ...
    if project == 'ghcnd':
        ...
        try:
            station_df = pd.read_fwf(station_url, widths=[11, 9, 10, 7, 3, 31, 4, 4, 6], names=[d for d in dtypes.keys()], converters=dtypes)
        except:
            statfile = Path(__file__).parent.joinpath("data/ghcnd-stations.txt")
            station_df = pd.read_fwf(statfile, widths=[11, 9, 10, 7, 3, 31, 4, 4, 6], names=[d for d in dtypes.keys()], converters=dtypes)
    ...
    if lon_bnds and lat_bnds:
        bbx_mask = station_df["lat"].between(lat_bnds[0], lat_bnds[1]) & station_df["lon"].between(lon_bnds[0], lon_bnds[1])
    else:
    ...
    for ii, ss in enumerate(station_list):
        ...
        ntry = 5
        while ntry > 0:
            errors = get_ghcn_raw(station_ids=ss, stationtype=prj_dict[project], outfolder=working_folder.joinpath('raw', str(ii)))
            if len(errors) == 0:
                break
            else:
                ss = errors
        ...
        infolders = [i for i in working_folder.joinpath('raw').iterdir() if i.is_dir() and i not in treated]
        if len(infolders) >= n_workers or ii+1 == len(station_list):
            ...
            for infolder in infolders:
                ...
                for vv, meta in var_attrs.items():
                    ...
                    outzarr = working_folder.joinpath('zarr', cf_var, f"{project}_{infolder.name}.zarr")
                    if not outzarr.exists() or overwrite:
                        var_attrs_new[vv] = meta
                if var_attrs_new:
                    dsall_vars = create_ghcn_xarray(infolder=infolder, varmeta=var_attrs_new, statmeta=station_df)
                    ...
                    for kk, vv in var_attrs_new.items():
                        ...
                        ds_corr[f"{cf_var}_q_flag"].attrs['long_name'] = f"Quality flag for {cf_var}"
                        ds_corr[f"{cf_var}_q_flag"].attrs['description'] = f"See the readme file for information of quality flag (QFLAG1) codes : {readme_url}"
...
def write_zarr(ds: xr.Dataset=None, outzarr:Path=None, overwrite:bool=False, chunks:dict=None):
    if not outzarr.exists() or overwrite:
        with ProgressBar():
