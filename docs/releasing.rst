...
From the miranda source folder we can enter into the docker container, providing access to the `src/miranda` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/miranda -w /src/miranda quay.io/pypa/manylinux_2_24_x86_64 bash
