...
--------------------------------------------------------------
Contributors to this version: Aslı Beşe (:user:`aslibese`), Trevor James Smith (:user:`Zeitsperre`).

Changes
^^^^^^^
* Updated ORRC global attributes to reflect the changes in dataset production processes. (:pull:`355`).
* Removed a few redundant functions (``miranda.io.concat_rechunk_zarr``, ``miranda.io.merge_rechunk_zarrs``). (:pull:`357`).
* ``convert_stationdata_bychunks`` has been renamed to ``convert_stationdata``. (:pull:`357`).
...
Internal changes
^^^^^^^^^^^^^^^^
* `miranda` now has guidance documents on acceptable usages of AI and the expected methods of AI usage disclosure. See the documentation for more details. (:pull:`359`).
* Fast-forwarded the cookiecutter with the latest changes. (:pull:`359`):
...
* ECMWF: Added support for the land-sea mask variable (`sftlf`) and lake fraction variable (`sftlaf`).
...
Internal changes
^^^^^^^^^^^^^^^^
* Harmonized use of _data_corrections vs .corrections for dataset conversion functions across the codebase. Replaced all calls to `miranda.convert.corrections` with the `miranda.convert._data_corrections` module.
