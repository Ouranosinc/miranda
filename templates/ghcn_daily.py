...
def main():
    ...
    for var1 in [v for v in working_folder.joinpath("zarr").iterdir() if v.is_dir()]:
        ...
        ds = xr.concat([xr.open_zarr(z, decode_timedelta=False) for z in inzarrs], dim='station').sortby(["station","time"])      
        outzarr = outfolder.joinpath(f'{var1.name}_day_NOAA_ghcnd_{ds.time[0].dt.strftime("%Y%m%d").values}-{ds.time[-1].dt.strftime("%Y%m%d").values}.zarr')
        # remove encoding
        for c in ds.coords:
            ds[c].load()
            ds[c].encoding ={}
        ...
        with ProgressBar():
            ds.chunk(dict(station=250, time=365*4+1)).to_zarr(outzarr.with_suffix('.tmp.zarr'), mode='w')     
