...
# Searches /dmf2 for the station, then calls the needed scripts to read and assembles the data using pandas
def find_and_extract_dly(path_station, rm_flags=False):
    ...
    # Find the CSV files
    station_files = sorted(glob.glob(path_station + '*.csv'))
...
def dly_to_netcdf(station, path_output):
    ...
    da.attrs['units'] = 'K'
    da.attrs['grid_mapping'] = 'regular_lon_lat'  # TODO: There is probably a better CF Convention for point-based data
...
def _read_multiple_eccc_dly(files, rm_flags):
    ...
    for f in range(len(files)):
        station_meta, data = _read_single_eccc_dly(files[f])
        if f == 0:
            datafull = data
        else:
...
def _read_single_eccc_dly(file):
