...
import xarray as xr
from xclim.core.units import str2pint, pint_multiply, convert_units_to, amount2rate
...
def parse_var_code(vcode):
    ...
    meta = {'freq': frequencies[freq][0], 'sampling': frequencies[freq][1], 'var_name': vcode[:-1]}
...
def list_tables(dbfile):
    ...
    if res.returncode != 0:
        raise ValueError(f'Calling mdb-tables on {dbfile} failed with code {res.returncode}: {res.stderr}')
    return res.stdout.upper().strip().split()


def read_table(dbfile, table, **kwargs):
    res = run(['mdb-export', '-T', "%Y-%m-%dT%H:%M:%S", dbfile, table], capture_output=True, encoding='utf-8', check=True)
    df =  pd.read_csv(
        StringIO(res.stdout),
        parse_dates=['DATE'],
        ...
        columns={'NO_SEQ_STATION': 'station', 'DATE': 'time'}
    ).drop(
        columns=['CODE_STATUT_DONNEE', 'STATUT_APPROBATION']
    ).set_index(['station', 'time']).VALEUR_DONNEE
    NaT = df.index.get_level_values('time').isnull().sum()
    if NaT > 0:
        logger.warning(f'{NaT} values dropped because of unparsable timestamps (probably data from before 1900).')
...
def convert_melcc(da, vcode, stations=None):
    ...
    if stations is not None:
        try:
            ...
            logger.error(f'The following station number were not found in the station list : {sorted(list(extra_stations))}')
            with open('unknown_stations.txt', 'a') as f:
                f.write('\n'.join(map(str, extra_stations)))
                f.write('\n')
            da = xr.merge([da, xr.Dataset().assign(station=stations)])[da.name].sel(station=da.station)
        else:
...
if __name__ == '__main__':
    argparser = ArgumentParser(description='Convert MS Access data to netCDF. Requires mdbtools to be installed. By default, a single netCDF is created for each data table.')
    argparser.add_argument('-v', '--verbose', help='Increase verbosity of the script.', action='store_true')
    argparser.add_argument('-c', '--concat', help='Merge all variables with the same frequency in individual datasets.', action='store_true')
    argparser.add_argument('-o', '--output', help='Output folder where to put the netCDFs.', default='.')
    argparser.add_argument('stations', help='Path to the excel file including the station information.')
    ...
    for database in folder.glob('*.mdb'):
        ...
        for table in tables:
            ...
            if args.concat:
                ...
                da.to_netcdf(output / f"MDDELCC_{data['freq']}_{da.name}_{da.melcc_code}.nc")
        ...
        for freq, variables in dss.items():
            ...
            for var_name, das in variables.items():
                if len(das) > 1:
                    ...
                    if n_conflicts > 0:
                        logger.warning(f'{n_conflicts} samples have multiple measurements for {var_name}/{freq}.')
                    da = xr.merge(
            ...
            ds.attrs.update(melcc_attrs['header'])
            ds.attrs['history'] = f"[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] Conversion from MS Access files to netCDF."
